...
# '''
# 提供接口,比如PDM调用后,上传文件并进行向量化上传到知识库
# '''
def upload_docs_custom_from_api(archiveName: str = Form(None, description="档案库名称", examples=["档案库名称"]),
...
#     自己添加方法-上传文件及向量化
# '''
def upload_docs_custom(files: List[UploadFile] = File(..., description="上传文件，支持多文件"),
                       knowledge_base_name: str = Form(..., description="知识库名称", examples=["samples"]),
                       override: bool = Form(False, description="覆盖已有文件"),
